...
import { countEarlyAccessMembers, getAccessSnapshot, loadReleaseSchedule, userFromCookieSession } from "@/lib/release-server";
...
export async function POST(request: Request) {
  ...
  if (access.isDeveloper) {
    return NextResponse.json({ ok: true, reason: "developer" });
  }
  ...
  const existing = (await userFromRequest(request)) ?? (await userFromCookieSession());
  ...
  if (existing) {
    if (access.isDeveloper || access.isMember) {
