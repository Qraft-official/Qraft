...
function canvasPagesFromPost(post: Post): CanvasPage[] {
  const pages = post.pages ?? [];
  if (!pages.length) return [emptyCanvasPage("page-1")];
...
export function EditProblemModal({
  ...
  useEffect(() => {
    ...
    const hand = post.solutionFormat === "handwriting";
    setInputMode(hand ? "hand" : "typed");
    setPages(canvasPagesFromPost(post));
  ...
  const save = async () => {
    ...
    const hasInk =
      images.some(Boolean) ||
      pages.some((p) => p.strokes.length > 0 || (p.texts?.length ?? 0) > 0 || !!p.backgroundImage);
