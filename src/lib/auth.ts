...
type ProfileRowLike = { id: string; name: string; handle: string | null; is_sample?: boolean | null };

export async function searchProfiles(query: string) {
  const q = query.trim().replace(/[%_,]/g, "").slice(0, 40);
  if (!q) return { profiles: [] as ProfileRowLike[], error: null as string | null };
  let res = await supabase
    .from("profiles")
    .select("id, name, handle, is_sample")
    .or(`name.ilike.%${q}%,handle.ilike.%${q}%`)
    .limit(24);
  if (res.error && /is_sample/i.test(res.error.message)) {
    const legacy = await supabase
      .from("profiles")
      .select("id, name, handle")
      .or(`name.ilike.%${q}%,handle.ilike.%${q}%`)
      .limit(24);
    if (legacy.error) return { profiles: [], error: legacy.error.message };
    return { profiles: (legacy.data ?? []) as ProfileRowLike[], error: null };
  }
  if (res.error) return { profiles: [], error: res.error.message };
  return { profiles: (res.data ?? []) as ProfileRowLike[], error: null };
}
