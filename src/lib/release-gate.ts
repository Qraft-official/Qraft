...
export function releasePhaseAt(nowMs: number, schedule: ReleaseSchedule): ReleasePhase {
  ...
  if (!Number.isFinite(startMs) || !Number.isFinite(publicMs) || !Number.isFinite(nowMs)) {
    return "prelaunch";
  }
...
export function remainingEarlyAccessSlots(memberCount: number, cap: number) {
  if (!Number.isFinite(memberCount) || !Number.isFinite(cap)) return 0;
  return Math.max(0, cap - memberCount);
}

export function blockedAccessSnapshot(phase: ReleasePhase = "prelaunch") {
  return {
    phase,
    canAccess: false,
    isAdmin: false,
