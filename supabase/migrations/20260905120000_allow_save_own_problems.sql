-- Personal bookmarks may include the author's own problems.
-- saved_problems is not a popularity metric; do not use self-saves in rankings.

alter table public.saved_problems drop constraint if exists saved_problems_no_self;
alter table public.saved_problems drop constraint if exists saved_problems_not_own;
alter table public.saved_problems drop constraint if exists saved_problems_user_ne_author;
...
begin
  ...
  -- Author may bookmark their own row. Do not compare author_id to uid.
